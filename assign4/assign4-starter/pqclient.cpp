...
Vector<DataPoint> topK(istream& stream, int k) {
    ...
    DataPoint cur;
    while(stream >> cur){
        // if pq.size() < k: enque all the DataPoint
        if (pq.size() < k){
            pq.enqueue(cur);
        }
        // else: keep the pq.size()==k, and make sure that the DataPoint in the pq has top priority value
        else {
            // pq.peek() has the lowest priority value
    ...
    // use pq.size() to create Vector, because pq.size() has the real count of DataPoint processed and stored
    Vector<DataPoint> result(pq.size());
    // result[0] has larger priority value, result[result.size()-1] has lower priority value
...
STUDENT_TEST("topK: time trial for k") {
    int n = 200000;
    for (int k = 100; k < 1000; k *= 2){
        Vector<DataPoint> input;
        fillVector(input, n);
//        stringstream stream = asStream(input);
        stringstream stream = asStream(1, n);
