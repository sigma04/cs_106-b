...
void PQArray::enqueue(DataPoint elem) {
    // if there is no space, allocate new spaces
    if (_numAllocated == _numFilled){
        // backup pointor of old space
        DataPoint* old = _elements;
        // allocate new space and double the size of space
        _numAllocated *= 2;
        _elements = new DataPoint[_numAllocated];
        // copy the data of old space into the new space
    ...
    // udpate _numFilled
    _numFilled++;
    // sort each elment by priority
    for (int i = _numFilled - 1; i > 0; i--){
        if (_elements[i].priority > _elements[i-1].priority) swap(i, i-1);
        else break;
    }
}
...
STUDENT_TEST("PQArrary enqueue timing analysis"){
    PQArray pq;
    int n = 10000;
    for (int k = 1; k < 8; k++){
