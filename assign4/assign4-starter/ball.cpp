...
//    }
    ...
    // move left and outside left edge, bounce to the right
    if (_x < 0 && _vx < 0) _vx *= -1;
    // move right and outside right edge, bounce to the left
    if (_x + SIZE > _window->getWidth() && _vx > 0) _vx *= -1;
    // move up and outside top edge, bounce to the bottom
    if (_y < 0 && _vy < 0) _vy *= -1;
    // move down and outside bottom edge, bounce to the top
    if (_y + SIZE > _window->getHeight() && _vy > 0) _vy *= -1;
}
...
PROVIDED_TEST("Animate bouncing balls in window for a while") {
    ...
    // set the seed of pseudorandom number for debugging
//    setRandomSeed(2000);
