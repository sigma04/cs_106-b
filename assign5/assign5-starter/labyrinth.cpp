 ...
 */
const string kYourName = "WANGWEI";

/* Change this constant to the path out of your labyrinth. */
const string kPathOutOfNormalMaze = "SESSWENNNEWSEESWS";
...
bool isPathToFreedom(MazeCell* start, string moves, Set<string> needs) {
    // finded sotre the state for all items,
    // compare finded to goal to judge if all item needed is collected
    Map<string, bool> finded, goal;
    for (string i : needs){
        finded[i] = false;
        goal[i] = true;
    }
    // get the item in the start location
    if (needs.contains(start->contents)){
        finded[start->contents] = true;
    }
    // iterate each step
    MazeCell* current = start;
    for (char route : moves){
        if (finded == goal) return true;
        // move one step
        switch (route) {
        case 'E':
        ...
        case 'N':
            ...
            break;
        default:
            error("undefined route exclude ESWN");
        }
        // current location is not allowed
        if (current == nullptr) return false;
        // if find one item needed
        if (needs.contains(current->contents)){
            finded[current->contents] = true;
        }
    }
    return finded == goal;
}
